...
**Last updated:** 2026-04-08 (late session — Rebuild Steps 1 through 5 shipped, Step 5b-lite stub + Pillar 1 refinements + Step 6-lite badge selector all in flight for final bundled commit; huddle at Trellix verification is next)
...
| **Step 5b-lite — Stubbed monolithic scoring call** | **ON DISK, UNCOMMITTED** (folded into the upcoming bundled commit). `scorer.score_selected_products` no longer makes a Claude call. Builds Product objects from discovery data directly + attaches fact drawer. Metadata fields (contacts, owning_org, lab_highlight, etc.) become empty during the rebuild window — restored at Step 5b proper via a dedicated lightweight metadata extractor. |
| **Pillar 1 refinements — all decisions locked 2026-04-08** | **ON DISK, UNCOMMITTED.** See §0f for the full list. Simulation override (5/12/0/25), fabric priority + optionality + VM reframing + container disqualifiers + M365 as first-class fabric (Tenant 25, Admin 18), Custom Cloud strength badge, No GCP Path badge, Pre-Instancing? suggestion rename, Large Lab deletion, Runs in VM rename (deferred to Step 5.5), badge coloring rules, badge naming rules (restricted list, ~25 chars / ~4 words, vendor acronyms only, Title Case, No X prefix for absence, no periods). |
| **Step 6-lite — Badge selector** | **ON DISK, UNCOMMITTED.** New `backend/badge_selector.py`. Deterministic Pillar 1 badge selection from facts + pass-through from GradedSignals for Pillars 2/3. Honors all the naming rules locked tonight. Wired into `intelligence.score()` after the cutover flip. |
| **Three Skillable capability docs filed** | **ON DISK, UNCOMMITTED.** `backend/knowledge/skillable_capabilities.json` enriched with `m365_tenants` (full tier matrix), `microsoft_office` (SPLA products), `byoc_custom_cloud_labs` (BYOC operational pattern with 10-phase workflow mapping to Pillar 1 dimensions), and `software_licensing` (Windows Client/Server, SQL Server, Tableau, and the potentially-supported Microsoft products list). |
| **All Chunk 1 Pillar 1 decisions** | See §0f for the complete list of every decision locked in the 2026-04-08 walkthrough. |
| **Trellix verification AFTER bundled commit** | Pending — HUDDLE POINT. Frank runs a forced-refresh Deep Dive on Trellix once the final commit lands. |
