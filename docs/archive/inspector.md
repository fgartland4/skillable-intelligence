...
**Solution Engineers** are Inspector's primary users, and the tool was built around their workflow. Before a discovery call or pilot proposal, an SE opens Inspector, enters the company name, selects the products most likely to become lab programs, and runs a full scoring pass. What they need from the output is specific and technical: per-product scores broken down by dimension, the Skillable Path determination (A1, A2, B, or C), the Essential Technical Resource that surfaces the single highest-priority open question before a pilot can start, and consumption estimates they can use to anchor a business case. Inspector replaces the hours of pre-call research an SE would otherwise do manually and surfaces the technical proof points — API surface, provisioning model, Marketplace presence, competitive lab signals — in a format that maps directly to Skillable's delivery architecture. Typical trigger: an AE brings in a new logo and the SE needs to get up to speed before the first technical conversation. If the goal is to scan a cold list of accounts rather than go deep on a single known one, start with Prospector — it runs Inspector's discovery pipeline across up to 25 companies in parallel and surfaces a ranked table in 5–8 minutes.
...
**Why:** Before a single dimension can be scored, Inspector needs to know what the company makes, how it deploys, and what kind of organizational readiness surrounds its products. The Research Layer is what produces that raw material — systematically, in parallel, and with enough depth to support evidence-backed scoring rather than inference from product category alone.
...
**Why:** Research and scoring produce a lot of structured data. The Results Layer is what makes that data useful — surfacing it in a format that supports a technical discovery conversation, an executive qualification conversation, and a decision on next steps, all from the same output.

**What:** A results dashboard that renders the full `CompanyAnalysis` — scores, evidence, paths, consumption table, contacts, and recommendations — plus an API endpoint that makes the full pipeline available programmatically for batch use cases like Prospector.

**How:** The dashboard renders when all parallel scoring calls complete. The API endpoint runs the full pipeline synchronously and returns the `CompanyAnalysis` as a single JSON payload, bypassing the streaming interface for callers that don't need it.
...
- **Per-product labability scores:** Dimension breakdown (Product Labability, Instructional Value, Organizational Readiness, Market Fit), total score, Skillable Path determination (Azure Cloud Slice / Custom API / BYOC / Hyper-V / VM / Simulation / Unknown), and any risk or qualification flags.
...
### Inspector and Designer

Inspector and Designer serve different personas and are accessed separately. Inspector is a Seller and SE tool — the output of an Inspector run informs the decision to pursue a lab program and prepares the SE for a technical discovery conversation. Designer is a program design tool used by instructional designers and program owners who are building the program after the commercial decision has been made.

When an account qualifies and a program is greenlit, the program owner opens Designer independently, entering the company and referencing the Inspector findings as context. The tools share the same underlying intelligence layer — Designer can be pointed at an existing `analysis_id` — but Sellers and SEs do not navigate from Inspector into Designer directly. That boundary keeps each tool focused on the right persona and the right moment in the workflow.
