...
**One fact, one badge.** Layer 1 is always a single `Vendor-Delivered Training` badge. Its evidence text names the specific modes (ILT, self-paced portal, vendor-run labs, bootcamps, published course calendar). Splitting Layer 1 into three separate badges for ILT/self-paced/labs is three labels for one fact and is a badging discipline violation.

**Trellix example:** Trellix has vendor-delivered training (ILT + self-paced + labs on Trellix Education Services) but NO formal ATP / ALP program. The honest answer emits ONE `Vendor-Delivered Training` badge as strong green (Layer 1 positive) AND `No Training Partners` (Layer 3 penalty). Layer 2 depends on whether Coursera / Pluralsight / CompTIA / SANS actually have Trellix-specific content; if not, emit `No Independent Training` as the Market Demand + Delivery Capacity cross-pillar penalty. Don't conflate "has training" with "has training partners" — they are two separate facts.
