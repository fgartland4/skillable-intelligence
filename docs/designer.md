...
Each checklist item has three states: **gray** (nothing captured yet), **yellow** (partially filled — the AI has found or inferred something but hasn't confirmed it, or the program owner has answered partially), and **green** (confirmed — enough information is locked in to proceed). Items move from gray to yellow as the AI reads uploaded documents and extracts signals. They move from yellow to green as the program owner confirms or the AI reaches sufficient confidence. The transitions happen in real time, as documents are processed and questions are answered. The program owner can watch items light up as the AI works. Skill Mapping is one checklist item among the nine — it follows the same gray → yellow → green progression as every other requirement.
...
Outlining and skill mapping are separate modes of focus, but they run in parallel — not in strict sequence. While the program owner and ID are building the outline, the AI is quietly proposing skill tags in the background. The outline itself stays clean; the mapping work doesn't interrupt or clutter the outlining experience. Progress is visible in the Blueprint checklist — *"Skill Mapping — 14 of 22 activities tagged"* — and the program owner or ID can flip into Skill Mapping Mode at any point to review and adjust what the AI has proposed. They don't have to wait for the outline to be fully approved. Outlining and mapping can interleave naturally.

When the program owner or ID enters Skill Mapping Mode, the proposed mappings are waiting for review — not starting from scratch. The AI has been working the whole time. The review is structured and designed for an instructional designer: full framework visible alongside the outline, mappings editable individually, and the ability to approve the complete map as a whole before it's locked. *"Configure Network Policies → NICE Work Role: Cyber Defense Analyst (KS0001, KS0002)"* — the ID reviews, confirms, or adjusts each one.

**Skill mapping review prompt on outline approval.** If a skill framework is configured and the AI has proposed tags but the program owner has not opened Skill Mapping Mode at all, clicking to approve the outline triggers a prompt: *"The AI has proposed skill tags for 22 activities — want to review them before moving to Phase 3?"* The program owner can review now or proceed anyway. This is not a blocker — it is a reminder. If they've opened Skill Mapping Mode even briefly, the prompt is suppressed; the assumption is they've seen what the AI proposed and made a deliberate choice.

**Tagging granularity — activity-level is the recommendation, not the rule.** The AI defaults to proposing tags at the activity level. This is almost always the right granularity: activities are the unit of learner action, and skills map most cleanly to what a learner actually does in a specific task. Some frameworks, however, are coarse enough that lab-level tagging is more natural than activity-level — and some programs or content teams simply don't need or want activity-level precision. Both are valid. Designer recommends activity-level tagging but does not enforce it. The ID decides what granularity is appropriate for their program and their organization's framework alignment goals.
