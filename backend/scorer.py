...
- **Cloud Slice - Azure**: Provisions isolated Azure subscriptions per learner. Supports ALL Azure services — no fixed whitelist; any service can be used after a Skillable Security Review. Also supports M365 tenant provisioning, Bicep templates (compiles to ARM at launch), and ARM JSON templates. This is Skillable's broadest cloud fabric — contrast with AWS, which has a specific supported services list.
