...
- Azure concurrent template deployment: Multiple resource templates in a lab deploy simultaneously — there is no sequencing. If a product deploys Azure resources with dependencies on each other (e.g., a VM that depends on a database being ready first), all dependent resources must be consolidated into a single template. Note this in the Configure bullet for products with multi-service Azure architectures.
