...
- **Skillable Datacenter**: Native Hyper-V, VMware ESX, and Docker. ANY software that installs on Windows or Linux runs here. Hyper-V and VMware ESX are both exceptionally mature fabrics — Skillable recommends Hyper-V as the primary choice due to extreme cost increases imposed by Broadcom on VMware licensing. When recommending this path, note Hyper-V specifically (not ESX) unless the customer has a specific ESX dependency. Docker is a separate, capable fabric — lightweight, application-focused rather than full-OS — but less mature than Hyper-V/ESX. Supported Docker configs: Linux containers in Docker for Linux, Windows containers in Docker for Windows, Linux containers in Docker for Windows on Azure Dv3/Dsv3/Ev3 VMs. NOT supported: nested virtualization scenarios. Important build consideration: large Docker images must be pre-downloaded into the lab during development to avoid delays during live courses. Prefer Hyper-V for workloads that can run either way; recommend Docker only when the product is genuinely container-native. **Datacenter VMs are significantly faster to launch and lower cost than cloud-hosted VMs — always prefer Hyper-V/ESX over Azure or AWS VMs when the product doesn't specifically require cloud infrastructure.**
...
⚠️ Note: Azure and AWS fabrics CAN run VMs (Azure Compute Gallery, AWS EC2 AMIs) — but only recommend cloud VMs when the product requires cloud-native infrastructure (Entra ID, Azure PaaS services, AWS services) that can't be replicated in Skillable's datacenter. If a product runs equally well on Hyper-V, always recommend Hyper-V for performance and cost.
...
- **Provision:** How Skillable provisions the environment. For VM/Hyper-V: describe the VM image, OS, and installed software. For Cloud Slice: describe how the Azure/AWS subscription is provisioned and what gets deployed into it. For Custom API: describe the vendor API call that spins up the learner environment.
...
- Azure concurrent template deployment: Multiple resource templates in a lab deploy simultaneously — there is no sequencing. If a product deploys Azure resources with dependencies on each other (e.g., a VM that depends on a database being ready first), all dependent resources must be consolidated into a single template. "Deploy in Background" flag on a template lets learners start the lab before it finishes deploying — useful for long-running provisioning (AI model loading, large DB restore, etc.). Note this in the Configure bullet for products with multi-service Azure architectures.
...
- ATP/Learning Partner program: +10 | Certification program: +6 | Gray market training: +4 | Events/conferences: +5 | Existing labs/sandboxes: +2. Cap 25.
