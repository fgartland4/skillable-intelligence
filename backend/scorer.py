...
- Azure-hosted VMs (Compute Gallery): Skillable can run VMs hosted in Azure (not in Skillable's datacenter) using Azure VM images from the Compute Gallery. This is the right path for products requiring GPU instances or Azure-specific hardware (e.g., Azure N-series GPU VMs for AI model training). VHD-based Azure VMs are deprecated — always reference Compute Gallery for new implementations. Note this option in the Provision bullet when a product needs GPU or hardware not available in Skillable's Hyper-V datacenter.
