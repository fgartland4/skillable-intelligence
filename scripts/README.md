...
| Script | What it does | Cost | When to use |
|---|---|---|---|
| `scripts/retrofit_acv.py` | Targeted gap-fill for cached discoveries. Three modes: `--mode holistic` (Option 2 holistic ACV), `--mode enrollments` (fills `annual_enrollments_estimate` on wrapper org products — ILT/Academic/ELP/GSI/VAR/Distributor/Industry Authority/Content Development), `--mode gap-fill` (fills `channel_partner_se_population` + `events_attendance` on analyses with cached Deep Dive). One Claude call per record — **not** full re-research. | ~$0.30–0.50 per company per mode | When a specific field is missing across cached data and we want to backfill without re-researching everything. |
| `scripts/compute_customer_potentials.py` | One-time pass for non-saturated known customers. Runs the holistic ACV call with `disable_known_customer_caps=True` to get each customer's un-clamped Potential. Writes `acv_potential` on their `_holistic_acv`. The anonymized calibration block then shows both `current_acv` and `acv_potential` per stage so Claude anchors prospects on Potential. | ~$0.30 per known customer | Re-run when the holistic prompt or calibration design changes meaningfully. |
| `backend/scripts/run_flagship_deep_dives.py` | Runs a full Deep Dive on each company's flagship product for companies that have no Deep Dive yet. Picks the flagship by `product_relationship="flagship"` → highest `rough_labability_score` → first product. Leaves companies with existing Deep Dives untouched. | ~$2.40 per company | When you want to give every company in the cache at least one real scored product (anchors Prospector in real data, not rough discovery guesses). |
